Polymer({
  ...
  submit: function () {
    ...
    if (this.invalid1 && this.invalid2 && this.post.version === undefined) {
      ...
      var lastChar = this.post.url.substr(-1);
      // Selects the last character
      if (lastChar === '/') {
        // If the last character is a slash
        this.post.url = this.post.url.substring(0, this.post.url.length - 1);
      }
      // remove the slash from end of string
      this.$.ajax.go();
      this.count = 0;
      if (this.interval) {
  ...
  computeUrl: function (post) {
    return post.url + '/rest/ping.view?u=' + post.user + '&p=' + post.pass + '&v=1.11.0&c=PolySonic&f=json';
  }
});
